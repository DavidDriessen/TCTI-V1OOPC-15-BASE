...
public:
	pin_in(hwlib::target::pin_in pin1,hwlib::target::pin_in pin2):pin1(pin1),pin2(pin2) {}
	bool get() {
		int out1 = 0;
		int out2 = 0;
		for(int i = 0; i<5; i++) {
		...
		hwlib::cout << hwlib::dec << out1 << " " << hwlib::dec << out2 << "\n";
		if(out1 > 2 && out2 > 2) {
			return 1;
		} else {
			return 0;
